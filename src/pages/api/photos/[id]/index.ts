...
async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  async function getPhotoById() {
    const data = await prisma.photo.findFirst(convertQueryToPrismaUtil(req.query, 'photo'));
    return res.status(200).json(data);
  }

  async function updatePhotoById() {
  ...
  async function deletePhotoById() {
    const data = await prisma.photo.delete({
      where: { id: req.query.id as string },
    });
    return res.status(200).json(data);
  }
